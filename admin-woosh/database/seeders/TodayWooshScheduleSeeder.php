...
class TodayWooshScheduleSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        if (!$halim || !$tegalluar || !$karawang || !$padalarang) {
            $this->command->error("Stasiun tidak lengkap di database. Silakan jalankan StationSeeder terlebih dahulu.");
            return;
        }

        $halimId = $halim->id;
        $tegalluarId = $tegalluar->id;
        $karawangId = $karawang->id;
        $padalarangId = $padalarang->id;

        // 2. Clear existing trips to avoid messy data
        DB::statement('SET FOREIGN_KEY_CHECKS=0;');
        DB::table('trips')->truncate();
        DB::statement('SET FOREIGN_KEY_CHECKS=1;');

        // 3. Define the base schedules
        $eastboundBase = [
            ...
            ['id' => 'G1049', 'dep' => '18:00', 'arr' => '18:47', 'mon_sat' => false],
        ...
        // Insert all trips
        $chunks = array_chunk($tripsToInsert, 100);
        foreach ($chunks as $chunk) {
            DB::table('trips')->insert($chunk);
        }

        $this->command->info("Berhasil menggenerate " . count($tripsToInsert) . " trips untuk hari ini (" . $dateStr . ") di semua rute!");
    }
}
